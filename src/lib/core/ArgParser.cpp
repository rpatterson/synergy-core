...
bool
ArgParser::parseServerArgs (ServerArgs& args, int argc,
...
bool
ArgParser::parseClientArgs (ClientArgs& args, int argc,
                            const char* const* argv) {
    setArgsBase (args);
    updateCommonArgs (argv);
...
bool
ArgParser::parsePlatformArg (ArgsBase& argsBase, const int& argc,
...
ArgParser::parseGenericArgs (int argc, const char* const* argv, int& i) {
    if (isArg (i, argc, argv, "-d", "--debug", 1)) {
        ...
        useDragDrop = false;
        LOG (
            (CLOG_INFO "ignoring --enable-drag-drop, not supported on linux."));
        ...
        LOG ((CLOG_INFO "--enable-crypto ignored, TLS is no longer supported "
                        "in Synergy Core"));
        return false;
...
bool
ArgParser::isArg (int argi, int argc, const char* const* argv,
...
bool
ArgParser::searchDoubleQuotes (String& command, size_t& left, size_t& right,
                               size_t startPos) {
    bool result = false;
    left        = String::npos;
    right       = String::npos;
...
String
ArgParser::assembleCommand (std::vector<String>& argsArray,
                            const String& ignoreArg, int parametersRequired) {
    String result;
