...
void
ServerApp::handleClientsDisconnected (const Event& /*unused*/,
                                      void* /*unused*/) {
...
ServerApp::closeServer (Server* server) {
    ...
    m_events->adoptHandler (m_events->forServer ().disconnected (),
                            server,
                            new TMethodEventJob<ServerApp> (
                                this, &ServerApp::handleClientsDisconnected));
...
void
ServerApp::updateStatus (const String& /*msg*/) {
}
...
ServerApp::openServer (Config& config, PrimaryClient* primaryClient) {
    ...
    try {
        ...
        m_events->adoptHandler (m_events->forServer ().screenSwitched (),
                                server,
                                new TMethodEventJob<ServerApp> (
                                    this, &ServerApp::handleScreenSwitched));
...
void
ServerApp::resetServer (const Event& /*unused*/, void* /*unused*/) {
...
int
ServerApp::runInner (int argc, char** argv, ILogOutputter* outputter,
                     StartupFunc startup) {
