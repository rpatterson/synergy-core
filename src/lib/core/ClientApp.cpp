            ...
            try {
                *m_serverAddress =
                ...
                if (!args ().m_restartable ||
                    e.getError () == XSocketAddress::kBadPort) {
...
#define WINAPI_INFO
    ...
    sprintf (buffer,
             "Usage: %s"
             " [--yscroll <delta>]" WINAPI_ARG HELP_SYS_ARGS HELP_COMMON_ARGS
             " <server-address>"
             "\n\n"
             ...
             "hostname\n"
             "must be the address or hostname of the server.  The port "
             "overrides the\n"
             "default port, %d.\n",
             args ().m_pname,
             kDefaultPort);
...
#elif WINAPI_XWINDOWS
    return new synergy::Screen (
        new XWindowsScreen (args ().m_display,
                            false,
...
ClientApp::handleClientFailed (const Event& e, void* /*unused*/) {
    auto* info = static_cast<Client::FailInfo*> (e.getData ());

    updateStatus (String ("Failed to connect to server: ") + info->m_what);
    if (!args ().m_restartable || !info->m_retry) {
        LOG ((CLOG_ERR "failed to connect to server: %s",
              info->m_what.c_str ()));
        m_events->addEvent (Event (Event::kQuit));
    } else {
        LOG ((CLOG_WARN "failed to connect to server: %s",
              info->m_what.c_str ()));
        if (!m_suspended) {
...
void
ClientApp::handleClientDisconnected (const Event& /*unused*/,
                                     void* /*unused*/) {
...
ClientApp::openClient (const String& name, const NetworkAddress& address,
    ...
    try {
        ...
        m_events->adoptHandler (m_events->forClient ().connectionFailed (),
                                client->getEventTarget (),
                                new TMethodEventJob<ClientApp> (
                                    this, &ClientApp::handleClientFailed));
...
ClientApp::closeClient (Client* client) {
    ...
    m_events->removeHandler (m_events->forClient ().connectionFailed (),
                             client);
...
int
ClientApp::runInner (int argc, char** argv, ILogOutputter* outputter,
                     StartupFunc startup) {
