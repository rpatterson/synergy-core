...
bool
ArchMultithreadPosix::waitCondVar (ArchCond cond, ArchMutex mutex,
                                   double timeout) {
...
ArchMultithreadPosix::newThread (ThreadFunc func, void* data) {
    ...
    if (status == 0) {
        status = pthread_create (&thread->m_thread,
                                 &attr,
                                 &ArchMultithreadPosix::threadFunc,
                                 thread);
...
void
ArchMultithreadPosix::setSignalHandler (ESignal signal, SignalFunc func,
                                        void* userData) {
    lockMutex (m_threadMutex);
...
ArchMultithreadPosix::startSignalHandler () {
    ...
    int status = pthread_attr_init (&attr);
    if (status == 0) {
        status = pthread_create (&m_signalThread,
                                 &attr,
...
ArchMultithreadPosix::erase (ArchThreadImpl* thread) {
    for (auto index = m_threadList.begin (); index != m_threadList.end ();
         ++index) {
        if (*index == thread) {
