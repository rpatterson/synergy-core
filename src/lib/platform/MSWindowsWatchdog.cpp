...
MSWindowsWatchdog::duplicateProcessToken (HANDLE process,
    ...
    BOOL tokenRet = OpenProcessToken (
        process, TOKEN_ASSIGN_PRIMARY | TOKEN_ALL_ACCESS, &sourceToken);
    ...
    if (m_elevateProcess || m_autoElevated ||
        ...
        if (!m_session.isProcessInSession ("winlogon.exe", &process)) {
            throw XMSWindowsWatchdogError (
                "cannot get user token without winlogon.exe");
        }
    ...
    while (m_monitoring) {
        try {
            ...
            if (m_processRunning && getCommand ().empty ()) {
                LOG ((CLOG_INFO
                      "process started but command is empty, shutting down"));
                shutdownExistingProcesses ();
                m_processRunning = false;
                continue;
            }
            ...
            if (m_processFailures != 0) {
                ...
                LOG ((CLOG_INFO "backing off, wait=%ds, failures=%d",
                      timeout,
                      m_processFailures));
                ARCH->sleep (timeout);
            }
            ...
            if (!getCommand ().empty () &&
                ((m_processFailures != 0) || m_session.hasChanged () ||
                 m_commandChanged)) {
                startProcess ();
            }
            ...
            if (sendSasFunc != NULL) {
                ...
                HANDLE sendSasEvent =
                    CreateEvent (NULL, FALSE, FALSE, "Global\\SendSAS");
                if (sendSasEvent != NULL) {

                    // use SendSAS event to wait for next session (timeout 1
                    // second).
                    if (WaitForSingleObject (sendSasEvent, 1000) ==
                        WAIT_OBJECT_0) {
...
MSWindowsWatchdog::startProcess () {
    if (m_command.empty ()) {
        throw XMSWindowsWatchdogError (
            "cannot start process, command is empty");
    }
    ...
    if (!createRet) {
        ...
        LOG ((CLOG_DEBUG
              "started process, session=%i, elevated: %s, command=%s",
...
BOOL
MSWindowsWatchdog::doStartProcess (String& command, HANDLE userToken,
    ...
    si.lpDesktop = "winsta0\\Default"; // TODO: maybe this should be \winlogon
                                       // if we have logonui.exe?
...
MSWindowsWatchdog::outputLoop (void*) {
    ...
    while (m_monitoring) {
        ...
        BOOL success = ReadFile (
            m_stdOutRead, buffer, kOutputBufferSize, &bytesRead, NULL);
...
MSWindowsWatchdog::shutdownProcess (HANDLE handle, DWORD pid, int timeout) {
    ...
    while (true) {
        ...
        GetExitCodeProcess (handle, &exitCode);
        if (exitCode != STILL_ACTIVE) {
            // yay, we got a graceful shutdown. there should be no hook in use
            // errors!
        ...
        if (entry.th32ProcessID != 0) {
            ...
            if (_stricmp (entry.szExeFile, "synergyc.exe") == 0 ||
                ...
                HANDLE handle = OpenProcess (
                    PROCESS_ALL_ACCESS, FALSE, entry.th32ProcessID);
