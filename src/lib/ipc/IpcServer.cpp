...
IpcServer::handleClientConnecting (const Event& /*unused*/, void* /*unused*/) {
    ...
    m_events->adoptHandler (m_events->forIpcClientProxy ().disconnected (),
                            proxy,
                            new TMethodEventJob<IpcServer> (
                                this, &IpcServer::handleClientDisconnected));

    m_events->adoptHandler (m_events->forIpcClientProxy ().messageReceived (),
                            proxy,
                            new TMethodEventJob<IpcServer> (
                                this, &IpcServer::handleMessageReceived));

    m_events->addEvent (Event (m_events->forIpcServer ().clientConnected (),
                               this,
                               proxy,
                               Event::kDontFreeData));
}
...
IpcServer::handleClientDisconnected (const Event& e, void* /*unused*/) {
    ...
    LOG ((CLOG_DEBUG "ipc client proxy removed, connected=%d",
          m_clients.size ()));
}
...
IpcServer::deleteClient (IpcClientProxy* proxy) {
    m_events->removeHandler (m_events->forIpcClientProxy ().messageReceived (),
                             proxy);
    m_events->removeHandler (m_events->forIpcClientProxy ().disconnected (),
                             proxy);
    delete proxy;
}
